...
public class TargetTracking : MonoBehaviour
{
    ...
    public RectTransform DistanceMeter;
    int meterStart = -520;
    int meterLenth = 1040;
    void Update()
    {
        if (target != null && trackingStarted)
        {
            ...
            int meterValue = meterStart  * ((int)distance / 100);
            Vector3 meterposition = DistanceMeter.anchoredPosition;
            meterposition.x = meterStart + meterValue;
            Debug.Log("Distance: " + distance + " MeterValue: " + meterposition);
            DistanceMeter.anchoredPosition = meterposition;

            int pointOfDistance = 0;
            for (int i = 0; i < targetTrackingIterations; i++)
            {
                pointOfDistance += targetTrackingThreshold;
                if (distance < pointOfDistance)
                {
                    playerScore.AddPointsForTracking(targetTrackingIterations - i);
                    break;
                }
            }
            RaceTime.text = "Score" + playerScore.GetScore().ToString("F2");
        }
        else
        {
            RaceTime.text = "";
